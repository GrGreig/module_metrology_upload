...
import csv
import re
import module_metrology_SFU as mm
...
def get_shield_dict(data_dictionary):
    """Computes the maximum height of the sheild box"""
    shield_dict = dict()
    for key, value in data_dictionary.items():
        if re.search(SHIELD_REGEX,key) :  
            shield_dict[key] = value[0]
    return sort_dict(shield_dict)
...
def get_file_data():
    ...
    try:
        ...
        print(2)
        sheild_dict = get_shield_dict(data_dict)
        print(3)
...
def save_data():
    ...
    if DATA_DICT['CAP'] != {} and DATA_DICT['SHIELD'] != {} :
        ...
        for key, point in DATA_DICT['SHIELD'].items() :
            file.write(f'{key}\t4\t{point[X]:0.4f}\t{point[Y]:0.4f}\t{point[Z]:0.4f}\n') 
    file.close()    
